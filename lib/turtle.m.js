var array = {
  ...
  threeSum: function threeSum() {
    ...
    for (var j = 1; j <= nums.length - 2; j++) {
      var i = 0,
          k = nums.length - 1;
      var min = nums[i] + nums[j] + nums[j + 1];
      var max = nums[j - 1] + nums[j] + nums[k];

      if (target >= min && target <= max) {
        while (i < j && j < k) {
          var curr = nums[i] + nums[j] + nums[k];

          if (curr === target) {
            var ret = [nums[i], nums[j], nums[k]];
...
export { DomUtil, EventBus, LazyMan, LazyManAsync, MyPromise, Schedule, array, buildTree, curry, isInstanceOf, maxStack, request, rock, travelMiddlerOrder, util };
